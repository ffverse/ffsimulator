...
league_info <- ff_league(conn = conn)

scoring_history <- ff_scoringhistory(conn = conn, season = base_seasons)

latest_rankings <- ffs_latest_rankings()

rosters <- ffs_rosters(conn = conn)

lineup_constraints <- ff_starter_positions(conn = conn)
...
cli::cli_process_start("Optimizing Lineups")
...
cli::cli_process_done("Optimizing Lineups...! {Sys.time()}")
